...
export class ExternalRecipeService {
  ...
  private async searchSpoonacular(query: string) {
    try {
      const response = await fetch(
        `${this.SPOONACULAR_BASE_URL}/complexSearch?query=${encodeURIComponent(query)}&apiKey=${this.SPOONACULAR_API_KEY}&number=10`
      );
      ...
      const data = await response.json() as { results: Array<{ id: number; title: string; image: string }> };
      const results = data.results || [];

      // Save found recipes to database
      const savedRecipes = await Promise.all(
        results.slice(0, 5).map((recipe) => this.fetchAndSaveRecipe(recipe.id))
      );
  ...
  private async fetchAndSaveRecipe(externalId: number) {
    try {
      ...
      // Fetch from API
      const response = await fetch(
        `${this.SPOONACULAR_BASE_URL}/${externalId}/information?apiKey=${this.SPOONACULAR_API_KEY}&includeNutrition=false`
      );
      ...
      const recipeData = await response.json() as {
        id: number;
        title: string;
        image: string;
        summary: string;
