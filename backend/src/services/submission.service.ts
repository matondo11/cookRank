...
export class SubmissionService {
  async submitRecipe(userId: string, recipeId: string, imageUrl: string, caption?: string) {
  ...
  async getSubmissions(page: number, limit: number, userId?: string, recipeId?: string) {
    const where: any = {};
  ...
  async updateSubmission(
    ...
    if (!submission) throw new Error('Submission not found');
    if (submission.userId !== userId) throw new Error('Not authorized to update this submission');
  ...
  async deleteSubmission(submissionId: string, userId: string) {
    ...
    if (!submission) throw new Error('Submission not found');
    if (submission.userId !== userId) throw new Error('Not authorized to delete this submission');

    await prisma.recipeSubmission.delete({ where: { id: submissionId } });
    return { message: 'Submission deleted successfully' };
  }

  private async awardExperiencePoints(userId: string, xpAmount: number) {
